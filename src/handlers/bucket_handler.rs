...
/// Handles bucket-level route requests and dispatches to bucket operations.
pub async fn bucket_handler(State(state): State<AppState>, request: Request) -> Response {
    let request_id = extract_request_id(&request);
    handle_bucket(state, request, &request_id)
        .await
        .unwrap_or_else(|error| error.into_response_with_request_id(&request_id))
}

/// Handles root list-buckets requests.
pub async fn list_buckets(State(state): State<AppState>, request: Request) -> Response {
    let request_id = extract_request_id(&request);
    handle_bucket(state, request, &request_id)
        .await
        .unwrap_or_else(|error| error.into_response_with_request_id(&request_id))
...
async fn handle_bucket(
    ...
    if !matches!(
        ...
        return Err(S3Error::InvalidRequest {
            message: "request does not match bucket handler".into(),
        });
    }
