...
/// Handles object-level route requests and dispatches to object operations.
pub async fn object_handler(State(state): State<AppState>, request: Request) -> Response {
    let request_id = extract_request_id(&request);
    handle_object(state, request, &request_id)
        .await
...
async fn handle_object(
    ...
    if !matches!(
        ...
        return Err(S3Error::InvalidRequest {
            message: "request does not match object handler".into(),
        });
    }
