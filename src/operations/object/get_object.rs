...
/// Handles the S3 GetObject operation.
pub struct GetObject;

impl GetObject {
    /// Retrieve the object at `(bucket, key)` and stream its bytes back to the
    /// caller.
    ///
    /// On success the response carries HTTP 200, standard object headers
    /// (`Content-Type`, `Content-Length`, `ETag`, `Last-Modified`), and a
    /// streaming body backed by the stored file.
    ///
    /// # Errors
    ///
    ...
    pub async fn execute(
        ...
        let (reader, len) = objects.get_object(&bucket, &key).await?;
